...
class Topics {
    ...
    @Bean
    fun ownerCommandsTopic() = NewTopic(TopicNames.OWNER_COMMANDS, 3, 3)

    @Bean
    fun dlqTopic() = NewTopic(TopicNames.DLQ, 3, 3)

    @Bean
    fun ownerEventsTopic() = NewTopic(TopicNames.OWNER_EVENTS, 3, 3)

    @Bean
    fun carEventsTopic() = NewTopic(TopicNames.CAR_EVENTS, 3, 3)
}
